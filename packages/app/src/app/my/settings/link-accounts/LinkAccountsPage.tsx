...
const convertErrorMessage = (errorCode: LinkAccountRequestErrorCode): string => {
  switch (errorCode) {
    case 'not-found': {
...
export const LinkAccountsPage: React.FC = () => {
  ...
  const { isLoading, accounts, currentAccount, linkAccountRequests, addLinkAccountRequest, updateLinkAccountRequest } =
                  ...
                  <li key={request.id} className="border-t border-back-2">
                    <div className="flex gap-2 px-4">
                      <div className="shrink grow py-2">
                        ...
                        {request.data.error && (
                          <p className="mt-1 text-xs">{convertErrorMessage(request.data.error)}</p>
                        )}
                      </div>
